from enigma_core.settings.settings import NUMBERS
import re


class RotorSettingsStringError(Exception):
...
class RotorSettingsStringValidator:
    ...
    def _extract_settings(self, pattern):
        ...
        rotor_settings = re.findall(regex, self.settings_string)
        rotor_settings = [c.upper() for c in rotor_settings]
    ...
    def _valid_settings(self, settings):
        ...
        if not settings:
            err_msg = f"{self.settings_string} are not valid rotor settings. Must be in range {settings_range}."
            raise RotorSettingsStringError(err_msg)
        ...
        if self.charset_flag == "N":
            for rotor_setting in settings:
                _rotor_setting = rotor_setting.rjust(2,'0')
                if _rotor_setting not in NUMBERS:
                    err_msg = f"Invalid rotor setting {rotor_setting}. Must be a number in range {settings_range}."
                    raise RotorSettingsStringError(err_msg)
                else:
                    valid_settings.append(_rotor_setting)
        if self.charset_flag == "L":
            for rotor_setting in settings:
                valid_settings.append(rotor_setting)
